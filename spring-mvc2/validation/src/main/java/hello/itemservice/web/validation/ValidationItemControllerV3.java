...
//    @PostMapping("/{itemId}/edit")
    public String editV1(@PathVariable("itemId") Long itemId, @Validated @ModelAttribute Item item, BindingResult bindingResult, Model model) {
        //특정 필드가 아닌 복합 룰 검증
        if(item.getPrice() != null && item.getQuantity() != null) {
            int resultPrice = item.getPrice() * item.getQuantity();
            if (resultPrice < 10000) {
    ...
    @PostMapping("/{itemId}/edit")
    public String editV2(@PathVariable("itemId") Long itemId
